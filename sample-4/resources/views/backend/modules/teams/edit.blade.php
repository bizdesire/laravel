...
@section('content')
    <div class="m-c-heading">
        ...
        <a href="{{ route('admin.team.index') }}" class="main-button m-b-rounded" title="view List">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <textarea name="description" class="form-control" required>{{ $data->designation }}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input type="file" name="image"  data-width="200" class="form-control pictureUpload" accept="image/*"
                                             >
                                            <div class="previewImage"> @if (!empty($data->image))
                                    ...
                                    <div class="col-sm-12" id="socialItemContainer">
                                        ...
                                        <?php $social_medias = json_decode($data->social_medias);
                                        $no = 0; ?>

                                        @if (!empty($social_medias))
                                            @foreach ($social_medias as $type => $value)
                                                <div class="row">
                                                    <div class="col-3">
                                                        <select name="social_medias[]" class="form-control" required>
                                                            <option value="">choose</option>
                                                            <option value="twitter"
                                                                {{ $type == 'twitter' ? 'selected' : '' }}>Twitter</option>
                                                            <option value="instagram"
                                                                {{ $type == 'instagram' ? 'selected' : '' }}>Instagram
                                                            </option>
                                                            <option value="linkedin"
                                                                {{ $type == 'linkedin' ? 'selected' : '' }}>linkedin
                                                            </option>
                                                        </select>
                                                    </div>
                                                    <div class="col-8">
                                                        <input type="text" name="urls[]" class="form-control"
                                                            value="{{ $value }}" placeholder="Enter url">
                                                    </div>
                                                    <div class="col-1">
                                                        @if ($no == 0)
